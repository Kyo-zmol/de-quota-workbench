...
function splitNums(t, decs) {
  for (const d of decs) {
    const one = new RegExp('^\\d+\\.\\d{' + d + '}$');
    if (one.test(t)) return [t];
    const many = new RegExp('\\d+\\.\\d{' + d + '}', 'g');
    const mm = t.match(many);
    if (mm && mm.length > 1 && mm.join('') === t) return mm;
  }
  return null;
}
...
(async () => {
  ...
  for (let pno = from; pno <= to; pno++) {
    ...
    for (const y of ys) {
      ...
      if (feeKey) { zone = 'fees'; const ncols = block.cols.length; const vals = []; for (const o of line) { const ns = norm(o.s).split(/\s+/).filter(t2 => isNum(t2) || t2 === '-'); for (const t2 of ns) { const sp = (t2 !== '-' && splitNums(t2, [2])) || [t2]; for (const q2 of sp) vals.push({ x: o.x, v: q2 === '-' ? null : toNum(q2) }); } } vals.sort((a2, b2) => a2.x - b2.x); const use = vals.length === ncols ? vals : (pendingFeeVals && vals.length === 0 ? pendingFeeVals : vals); pendingFeeVals = null; if (use.length === ncols) use.forEach((v, i) => { block.fees[i][feeKey] = v.v; }); else for (const v of use) { const ci = colIdx(v.x); if (ci >= 0) block.fees[ci][feeKey] = v.v; } continue; }
      ...
      if (zone === 'cons') {
        ...
        if (unitO) pendingSup = '';
        const qvals = []; for (const o of qtyParts) { const ns = norm(o.s).split(/\s+/).filter(t2 => isNum(t2) || t2 === '-'); for (const t2 of ns) { const sp = (t2 !== '-' && splitNums(t2, [3, 2])) || [t2]; for (const q2 of sp) qvals.push({ x: o.x, v: q2 === '-' ? null : toNum(q2) }); } } qvals.sort((a2, b2) => a2.x - b2.x); if (qvals.length === block.cols.length) qvals.forEach((v, i) => { entry.qty[i] = v.v; }); else for (const v of qvals) { const ci = colIdx(v.x); if (ci >= 0) entry.qty[ci] = v.v; }
